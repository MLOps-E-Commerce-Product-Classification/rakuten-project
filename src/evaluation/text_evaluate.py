...
EVAL_LOGGER = setup_logger("text_evaluation", LOG_PATH / "text_evaluation.log")
...
def compute_classification_metrics(
    ...
    if compute_per_class_f1:
        ...
        metrics["per_class_precision"] = {
            int(label): float(score) for label, score in zip(labels, per_class_prec)
        }
        metrics["per_class_recall"] = {
            int(label): float(score) for label, score in zip(labels, per_class_rec)
        }
...
def measure_inference_performance(
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        measured_num_samples += input_ids.size(0)
...
def evaluate_model(
    ...
    """Evaluate a trained text model on a dataset."""
    del criterion  # kept for symmetry with image evaluation interface
    ...
    for batch in dataloader:
        ...
        if "productid" in batch:
            all_product_ids.extend(batch["productid"])
...
def save_evaluation_results(
    results: dict,
    output_path: str | Path = RESULTS_PATH / "text_evaluation_results.json",
) -> None:
